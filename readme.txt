=== JSON Dashboard Infos ===

Plugin Name: JSON Dashboard Infos
Plugin URI: https://github.com/rcarlier/json-dashboard-infos
Description: Expose informations from your Wordpress, in JSON format, to allow you to create centralized dashboards (not included ...)
Tags: json, dashboard, api, export data, stats, comments
Author: Richard Carlier
Author URI: https://carlier.biz/
Contributors: richnou
Requires at least: 4.9
Tested up to: 5.5.1
Requires PHP: 5.3.29
Stable tag: 1.0.3
License: GPL v3 or later
License URI: http://www.gnu.org/licenses/

Expose informations from your Wordpress, in JSON format, to allow you to create centralized dashboards (not included ...)

== Description ==

The basic idea is to be able to build a dashboard centralizing information from several Wordpress, and thus monitor them all at once ...

Warning : this plugin is mostly technical...

Demonstrations (in video, and in French)

* [Presentation](https://www.youtube.com/watch?v=JCyXrJ3G6-of)
* [Installation, configuration](https://www.youtube.com/watch?v=7T44xJj3jPM)
* [Dashboard with React](https://www.youtube.com/watch?v=cfV-TfHQCyw)


== Installation ==

1. Go to your admin area and select Plugins → Add New from the menu.
2. Search for "JSON Dashboard Infos" (by Richard Carlier).
3. Click install.
4. Click activate.
6. Navigate to Settings > JSON Dashboard Infos.
7. Select options you want, and validate
8. Navigate to Settings > Permalink, and just validate (if needed)

== Screenshots ==

1. Admin view : options, previews, url of json
2. the JSON !
3. example of a dashboard with 15 wordpress using the plugin (code for this dashboard not provided)

== ChangeLog ==

= Version 1.0.4 =

* Minor correction.

= Version 1.0.3 =

* Bug resolution : infos on "Updates" now work correctly

= Version 1.0.2 =

* Better translation in French.
* Add video link (in french).

= Version 1.0.1 =

* Minor correction.

= Version 1.0.0 =

* Initial release.

= Upgrade Notice =

Support for WordPress 5.3
